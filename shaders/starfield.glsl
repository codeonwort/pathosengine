...
vec3 EquirectangularToCube(vec2 uv) {
	uv -= 0.5;
	uv *= 1.0 / vec2(0.1591, 0.3183);

	float y = sin(uv.y);
	float x = sqrt(1 - y*y) * cos(uv.x);
	float z = x * tan(uv.x);
...
float stars(vec3 seed, float intensity) {
	...
	float x = supernoise3dX(seed * 3700.0) * (0.5 + 0.5 * supernoise3dX(seed * 2.0));
...
vec3 stars(vec3 dir) {
	...
	const float baseZ = 0.5;
	float zFactor = abs(baseZ - dir.z);
	float xFactor = pow(max(0.0, dir.x - baseX), 2.0);

	float galaxydust = smoothstep(0.1, 0.5, (1.0 / (1.0 + 16.0 * zFactor)) * fbmHI3d(dir * 30.0, 3.0) * xFactor);
	float galaxydust2 = smoothstep(0.1, 0.5, (1.0 / (1.0 + 27.0 * zFactor)) * fbmHI3d(dir * 20.0, 3.0) * xFactor);

	//float galaxydust = smoothstep(0.1, 0.5, (1.0 / (1.0 + 30.0 * zFactor)) * fbmHI3d(dir * 30.0, 3.0));
	//float galaxydust2 = smoothstep(0.1, 0.5, (1.0 / (1.0 + 20.0 * zFactor)) * fbmHI3d(dir * 20.0, 3.0));
	...
	vec3 dustinner = 0.7 * vec3(0.9, 0.8, 0.8);
	vec3 dustouter = 0.7 * vec3(0.2, 0.1, 0.0);
	vec3 innermix = mix(dustinner, starscolor, 1.0 - galaxydust);
	vec3 allmix = mix(dustouter, innermix, 1.0 - galaxydust2);
	vec3 bloom = 1.6 * dustinner * (1.0 / (1.0 + 30.0 * abs(dir.z))) * fbmHI3d(dir * 3.0, 3.0) * (1.0 - abs(dir.x));
	return allmix;
...
void main() {
	...
	uv.x += 0.01 * (0.5 - configurablenoise(1000.0 * vec3(uv, 2.0), 126.3, 62.42));
	uv.y += 0.01 * (0.5 - configurablenoise(1000.0 * vec3(uv, 1.0), 626.1, 33.27));

	vec3 dir = EquirectangularToCube(uv);

	//out0 = vec4(vec3(fbmHI3d(dir * 10.0, 3.0)), 1.0);
